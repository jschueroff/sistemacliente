...
function carregarLista() {
    ...
    while ($d = mysqli_fetch_array($res, MYSQLI_BOTH)) {
        $saida  = $saida. "<tr>"
                . "  <td style='width:25%'><img class=thumb src='/crud-master/imagens/".$d['foto']."' /></td>"
                . "  <td>"
                ...
                . "  <td style='width:10%'><input type=button class='button delete' value=Excluir onClick='excluirRegistro(\"".$d['id']."\");'></td>"
                . "</tr>";
    }
    $saida = $saida. "</table>";
...
//*****************************************************************************
// Metodo que salva ou atualiza no cadastro
//
function salvarForm() {
    //var_dump($_POST);
    if ( ! isset( $_POST ) || empty( $_POST ) ) {
    ...
    //prepara e executa sql inserir os dados
...
function validarForm( $id, $nome, $email, $telefone, $foto ) {
    ...
    //validar campo foto
    if ( empty( $foto ) ) {        //return "Campo Foto deve ser preenchido.";
    }
